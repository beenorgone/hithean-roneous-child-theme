...
function hithean_render_product_nutrition_label(): void
{
    ...
    <section class="product-nutrition-label" aria-label="Nutrition label">
        <button class="product-nutrition-label__trigger" type="button" aria-haspopup="dialog" aria-controls="<?php echo esc_attr($modal_id); ?>" aria-expanded="false"><?php esc_html_e('Xem bảng dinh dưỡng', 'hithean-product-metabox'); ?></button>
    </section>
...
add_action('woocommerce_after_single_product_summary', 'hithean_render_product_nutrition_label', 5);
...
function hithean_product_nutrition_label_assets(): void
{
    ...
    $script = "document.addEventListener('click',function(e){var t=e.target.closest('.product-nutrition-label__trigger');if(t){var m=document.getElementById(t.getAttribute('aria-controls'));if(!m)return;m.hidden=false;m.setAttribute('aria-hidden','false');t.setAttribute('aria-expanded','true');m.querySelector('.product-nutrition-label__close').focus();return}var m=e.target.closest('.product-nutrition-label__modal');if(!m)return;if(e.target.closest('[data-nutrition-label-close]')){m.hidden=true;m.setAttribute('aria-hidden','true');var o=document.querySelector('.product-nutrition-label__trigger[aria-controls=\\\"'+m.id+'\\\"]');if(o){o.setAttribute('aria-expanded','false');o.focus()}return}var b=e.target.closest('[data-nutrition-label-thumb]');if(!b)return;var i=b.getAttribute('data-nutrition-label-thumb');m.querySelectorAll('[data-nutrition-label-image]').forEach(function(x){x.hidden=x.getAttribute('data-nutrition-label-image')!==i});m.querySelectorAll('[data-nutrition-label-thumb]').forEach(function(x){var a=x===b;x.setAttribute('aria-selected',a?'true':'false');x.classList.toggle('is-active',a)})});document.addEventListener('keydown',function(e){if(e.key==='Escape'){var m=document.querySelector('.product-nutrition-label__modal:not([hidden])');if(m)m.querySelector('[data-nutrition-label-close]').click()}});";
