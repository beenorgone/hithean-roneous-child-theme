...
function hithean_cro_optional_ai(array $insights, array $rows): array {
    ...
    $system = 'Bạn là CRO analyst cho e-commerce Việt Nam. Chỉ viết lại ngắn gọn giả thuyết và thử nghiệm từ số liệu tổng hợp được cung cấp. Không bịa số liệu, không khẳng định nhân quả, không đề xuất dark pattern. Trả JSON array có tối đa 5 object: index (0-based), hypothesis, test.';
    $provider = (string) $settings['ai_provider'];
    $cfg = ['provider' => $provider, 'model' => (string) $settings['ai_model'], 'pinned' => $provider !== '' && $provider !== 'auto']; // chọn provider cụ thể → không fallback
    $reply = theme_ai_feature_call($cfg, static function (string $provider, string $model, string $api_key) use ($system, $safe_data) {
        return theme_ai_call_provider($provider, $system, [['role' => 'user', 'content' => $safe_data]], 1100, $model, ['api_key' => $api_key]);
    });
...
function hithean_cro_admin_post(): void {
    ...
    if ($action === 'save_settings') { $old = hithean_cro_settings(); $old['property_id'] = sanitize_text_field((string) ($_POST['property_id'] ?? '')); $old['lark_webhook'] = hithean_cro_lark_webhook((string) ($_POST['lark_webhook'] ?? '')); $old['landing_paths'] = sanitize_textarea_field((string) ($_POST['landing_paths'] ?? '')); $old['ai_enabled'] = !empty($_POST['ai_enabled']) ? '1' : '0'; $old['ai_provider'] = in_array(sanitize_key((string) ($_POST['ai_provider'] ?? 'auto')), ['auto','claude','gemini','gemini_billing','openai','qwen'], true) ? sanitize_key((string) $_POST['ai_provider']) : 'auto'; $old['ai_model'] = sanitize_text_field((string) ($_POST['ai_model'] ?? '')); update_option(HITHEAN_CRO_OPTION, $old, false); $notice = 'saved'; }
