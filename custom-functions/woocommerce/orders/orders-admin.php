...
function hithean_order_admin_render_quick_action_modal(): void
{
    ...
    <style>
        ...
        .hithean-order-modal__field--checkbox { display:flex; grid-template-columns:auto 1fr; align-items:flex-start; gap:9px; margin-top:14px; padding:12px; border:1px solid #c7e7dc; border-radius:8px; background:#f0fdf8; }
        .hithean-order-modal__field--checkbox input { width:auto; margin:3px 0 0; }
        .hithean-order-modal__field--checkbox small { display:block; margin-top:4px; color:#52606d; font-weight:400; line-height:1.45; }
    ...
    (function () {
        ...
        function fieldsFor(mode, data) {
            ...
            if (mode === 'images') return '<input type="hidden" name="ueif_order_id" value="' + data.id + '"><label class="hithean-order-modal__field">Ảnh xuất kho (tối đa 5 ảnh)<input type="file" name="ueif_images[]" accept="image/jpeg,image/png" multiple required></label><label class="hithean-order-modal__field hithean-order-modal__field--checkbox"><input type="checkbox" name="ueif_ai_check" value="1" checked><span><strong>Gọi AI kiểm tra ảnh sau khi upload</strong><small>Ảnh sẽ được gửi tới AI provider đã cấu hình để đối chiếu mã đơn, sản phẩm và số lượng; kết quả chỉ hỗ trợ kiểm tra, không tự xác nhận xuất kho.</small></span></label><div class="hithean-order-modal__checklist">' + cfg.checklist.map(function (item, index) { return '<label><input type="checkbox" name="checklist_' + index + '">' + item + '</label>'; }).join('') + '</div>';
        ...
        form.addEventListener('submit', function (event) {
            ...
            if (active.mode === 'images') { var files = form.querySelector('[type="file"]').files, aiCheck = form.elements.ueif_ai_check && form.elements.ueif_ai_check.checked; if (!files.length) return notice('Hãy chọn ảnh xuất kho.', true); if (files.length > 5) return notice('Chỉ được upload tối đa 5 ảnh mỗi lần.', true); if (form.querySelectorAll('.hithean-order-modal__checklist input:not(:checked)').length) return notice('Hãy xác nhận đầy đủ checklist trước khi upload.', true); action = 'ajax_upload_images'; nonce = cfg.nonces.exportImages; loading = aiCheck ? 'Đang upload và AI kiểm tra...' : 'Đang upload...'; }
