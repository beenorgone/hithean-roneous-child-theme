...
function thean_lw_is_eligible_page(): bool
{
    ...
    $post_name = is_object($slug) && !empty($slug->post_name) ? sanitize_title($slug->post_name) : '';
...
function thean_lw_render_widget(): void
{
    ...
    ?>
    <div id="thean-lw-root" class="thean-lw" data-context="<?php echo esc_attr(thean_lw_page_context()); ?>">
            ...
            <div class="thean-lw-panel">
                ...
                <div class="thean-lw-grid">
                    <div class="thean-lw-wheel-wrap">
                        <div class="thean-lw-wheel" aria-hidden="true">
                            <span>5%</span><span>Freeship</span><span>10%</span><span>20K</span><span>30K</span><span>Ưu đãi</span>
                        </div>
                    </div>
                    <div class="thean-lw-content">
                        <p class="thean-lw-kicker">Ưu đãi riêng cho lượt ghé này</p>
                        <h2 id="thean-lw-title">Quay để giữ mã trong 24 giờ</h2>
                        <p class="thean-lw-copy">Bạn có tối đa 3 lượt quay. Chọn ưu đãi phù hợp nhất rồi để lại email hoặc số điện thoại để nhận mã.</p>
                        <p class="thean-lw-spins" data-thean-lw-spins>Đang tải...</p>
                        <div class="thean-lw-result" data-thean-lw-result hidden></div>
                        <div class="thean-lw-actions">
                            <button class="button thean-lw-spin-btn" type="button" data-thean-lw-spin>Quay ngay</button>
                            <button class="button thean-lw-secondary" type="button" data-thean-lw-save hidden>Lưu ưu đãi này</button>
                        </div>
                        <form class="thean-lw-form" data-thean-lw-form hidden>
                            ...
                            <button class="button" type="submit">Nhận mã ưu đãi</button>
                            <p class="thean-lw-form-note">Mã chỉ được tạo sau bước này và có hiệu lực 24 giờ.</p>
                        </form>
...
function thean_lw_ajax_spin(): void
{
    ...
    if (!thean_lw_rate_limit('spin_' . thean_lw_session_id(), 6, HOUR_IN_SECONDS)) {
...
function thean_lw_ajax_claim(): void
{
    ...
    $token = thean_lw_verify_token(isset($_POST['claim_token']) ? sanitize_text_field(wp_unslash((string) $_POST['claim_token'])) : '');
    if ($token === '') {
    ...
    if (!thean_lw_rate_limit('claim_ip_' . thean_lw_client_ip_hash(), 5, HOUR_IN_SECONDS)) {
        wp_send_json_error(['message' => 'Bạn đã yêu cầu quá nhiều lần. Vui lòng thử lại sau.'], 429);
    }
...
function thean_lw_pick_reward(): ?array
{
    ...
    $total = array_sum(array_map(static function ($reward) {
        return (int) $reward['frequency'];
    }, $pool));
