...
$product_count = isset($term->count) ? max(0, (int) $term->count) : 0;
...
// Cache phụ thuộc nội dung mô tả + thumbnail id + số sản phẩm + cờ use_expand
$cache_key = 'term_desc_block_' . $term->term_id . '_' . md5($raw_desc . '|' . $thumb_id . '|' . $product_count . '|' . (int) $use_expand);
    ...
    if ($term_desc) {
        if ($use_expand) {
            ...
            $desc_block = '<h2 style="margin-top:0; text-align:center;">' . esc_html($term->name) . '</h2>'
                . '<p class="taxonomy-product-count">' . esc_html(sprintf(_n('%s sản phẩm đang có', '%s sản phẩm đang có', $product_count, 'roneous'), number_format_i18n($product_count))) . '</p>'
        ...
        // Không có mô tả
        $desc_block  = '<h2 style="margin-top:0; text-align:center;">' . esc_html($term->name) . '</h2>';
        $desc_block .= '<p class="taxonomy-product-count">' . esc_html(sprintf(_n('%s sản phẩm đang có', '%s sản phẩm đang có', $product_count, 'roneous'), number_format_i18n($product_count))) . '</p><div id="section-product-ctas" class="b-single-product-ctas" style="text-align:center;">'
            . '<a class="button--dark-blue-reverse" href="#main-content"><i class="icon-products ti-angle-double-down"></i>Xem sản phẩm</a><a target="_blank" class="button--light-blue" href="https://m.me/61558663706094"><i class="icon-facebook ti-facebook"></i>Chat với The An</a>'
            . '</div>';
    }
...
<section class="block-product-desc" aria-labelledby="term-desc-title">
    <style>
        ...
        .block-product-desc .taxonomy-product-count{margin:0 0 18px;text-align:center;color:#6b5b4d;font-weight:600}
